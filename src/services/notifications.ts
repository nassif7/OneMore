...
export const computePattern = async (): Promise<TSmokingPattern> => {
  ...
  return {
    ...
    avgFirstCigMinutes:
      firstMins.length > 0
        ? firstMins.reduce((a, b) => a + b, 0) / firstMins.length
        : null,
    avgLastCigMinutes:
      lastMins.length > 0
        ? lastMins.reduce((a, b) => a + b, 0) / lastMins.length
        : null,
  };
};
...
export const scheduleFirstCigNotification = async (): Promise<void> => {
  ...
  tomorrow.setHours(
    Math.floor(pattern.avgFirstCigMinutes / 60),
    Math.round(pattern.avgFirstCigMinutes % 60),
    0,
    0,
  );

  const secondsUntilFirst = Math.round(
