...
export default function AnimatedSplashScreen({ onFinished }: IProps) {
  ...
  useEffect(() => {
    ...
    barHeight.value = withDelay(
      50,
      withTiming(52, { duration: 320, easing: sharp }),
    );

    // 250ms — shadow drops
    shadowOpacity.value = withDelay(
      250,
      withTiming(1, { duration: 300, easing: sharp }),
    );
    shadowTranslate.value = withDelay(
      250,
      withTiming(SHADOW_OFFSET, { duration: 300, easing: sharp }),
    );
    ...
    panelRotate.value = withDelay(
      300,
      withSpring(0, { damping: 14, stiffness: 140 }),
    );

    // 750ms — cross H sweeps
    crossHScale.value = withDelay(
      750,
      withTiming(1, { duration: 380, easing: sharp }),
    );

    // 870ms — cross V sweeps
    crossVScale.value = withDelay(
      870,
      withTiming(1, { duration: 380, easing: sharp }),
    );
    ...
    stampTranslate.value = withDelay(
      1050,
      withTiming(0, { duration: 300, easing: E.out(E.quad) }),
    );

    // 1100ms — ticker reveals
    tickerReveal.value = withDelay(
      1100,
      withTiming(1, { duration: 280, easing: sharp }),
    );
    ...
    titleTranslate.value = withDelay(
      1200,
      withTiming(0, { duration: 420, easing: E.out(E.quad) }),
    );
